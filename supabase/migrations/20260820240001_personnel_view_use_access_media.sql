-- Fase 1: la vista personnel_with_status deja de depender de `cards` y calcula
-- el estado a partir de access_media + access_media_types (name = tipo legible).
  ...
  WITH person_ready_cards AS (
    SELECT p_1.id,
      count(DISTINCT amt.name) FILTER (
        WHERE amt.name = ANY (ARRAY['P2000'::text, 'KONE'::text])
          AND am.status = 'active'::text
