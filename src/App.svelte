<script lang="ts">
    ...
    onMount(async () => {
        // Viewport (teclado virtual) para ajustes móviles.
        viewport.init();
        ...
        supabase.auth.onAuthStateChange(async (event, newSession) => {
            if (newSession) {
                ...
                if (event === 'SIGNED_IN' && appInitialized && userState.profile?.id === newSession.user.id) {
                    return;
                }
...
<a
    href="#main"
    class="sr-only focus:not-sr-only focus:fixed focus:top-3 focus:left-3 focus:z-[200] focus:px-4 focus:py-2.5 focus:rounded-xl focus:bg-slate-900 focus:text-white focus:text-sm focus:font-bold focus:shadow-lg"
>
    Saltar al contenido
</a>
    ...
    <div class="min-h-screen bg-slate-50 flex items-center justify-center p-4">
        <div class="flex flex-col items-center max-w-sm text-center">
            ...
            <p class="text-slate-500 mb-8 text-sm">
                No pudimos cargar los datos iniciales de la aplicación. Por favor, verifica tu conexión a
        ...
        <svelte:boundary>
            ...
            {#snippet failed(_error, reset)}
                <div class="flex min-h-[60vh] flex-col items-center justify-center p-6 text-center">
                    ...
                    <h2 class="text-xl font-bold text-slate-900 mb-2">Algo salió mal en esta vista</h2>
                    <p class="text-slate-500 mb-8 text-sm max-w-sm">
                        Ocurrió un error al renderizar la pantalla. Puedes reintentar sin recargar toda la
                        aplicación.
                    </p>
                    <button
                        class="flex items-center gap-2 px-6 py-3 bg-slate-900 text-white font-bold rounded-xl hover:bg-slate-800 transition-all active:scale-95 shadow-lg shadow-slate-900/10"
