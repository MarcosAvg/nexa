...
export const catalogService = {
    ...
    async saveBuilding(
        id: number | null,
        payload: { name: string; floors: string[] },
        ...
        return withErrorHandling(async () => {
            ...
            if (id) {
                ...
                await HistoryService.log('SYSTEM', id, 'UPDATE_CATALOG', {
                    message: `Edificio actualizado: ${payload.name}`,
                    entityName: `Edificio: ${payload.name}`,
                });
            } else {
                const sortOrder = await getNextSortOrder('buildings');
                const { data, error } = await supabase
                    .from('buildings')
                    .insert([{ name: payload.name, sort_order: sortOrder }])
                    .select()
                    .single();
                if (error) throw error;
                buildingId = data.id;
                await HistoryService.log('SYSTEM', data.id, 'CREATE_CATALOG', {
                    message: `Edificio creado: ${payload.name}`,
                    entityName: `Edificio: ${payload.name}`,
                });
            }
    ...
    async saveDependency(id: number | null, payload: { name: string }) {
        return withErrorHandling(async () => {
            if (id) {
                ...
                if (error) throw error;
                await HistoryService.log('SYSTEM', id, 'UPDATE_CATALOG', {
                    message: `Dependencia actualizada: ${payload.name}`,
                    entityName: `Dependencia: ${payload.name}`,
                });
            } else {
                ...
                if (error) throw error;
                await HistoryService.log('SYSTEM', data.id, 'CREATE_CATALOG', {
                    message: `Dependencia creada: ${payload.name}`,
                    entityName: `Dependencia: ${payload.name}`,
                });
            }
    ...
    async saveAccess(id: number | null, payload: { name: string; buildingId?: number }) {
        return withErrorHandling(async () => {
            if (id) {
                ...
                if (error) throw error;
                await HistoryService.log('SYSTEM', id, 'UPDATE_CATALOG', {
                    message: `Acceso especial actualizado: ${payload.name}`,
                    entityName: `Acceso especial: ${payload.name}`,
                });
            } else {
                ...
                if (error) throw error;
                await HistoryService.log('SYSTEM', data.id, 'CREATE_CATALOG', {
                    message: `Acceso especial creado: ${payload.name}`,
                    entityName: `Acceso especial: ${payload.name}`,
                });
            }
    ...
    async saveSchedule(id: number | null, payload: { name: string; days: string[] }) {
        return withErrorHandling(async () => {
            if (id) {
                ...
                if (error) throw error;
                await HistoryService.log('SYSTEM', id, 'UPDATE_CATALOG', {
                    message: `Horario actualizado: ${payload.name}`,
                    entityName: `Horario: ${payload.name}`,
                });
            } else {
                ...
                if (error) throw error;
                await HistoryService.log('SYSTEM', data.id, 'CREATE_CATALOG', {
                    message: `Horario creado: ${payload.name}`,
                    entityName: `Horario: ${payload.name}`,
                });
            }
    ...
    async saveMediaType(
        ...
        return withErrorHandling(async () => {
            ...
            if (id) {
                ...
                if (payload.requires_programming !== undefined)
                    update.requires_programming = payload.requires_programming;
                if (payload.requires_responsiva !== undefined)
                    update.requires_responsiva = payload.requires_responsiva;
                if (payload.requires_identifier !== undefined)
                    update.requires_identifier = payload.requires_identifier;
                const lock = await updateWithLock('access_media_types', id, update, expectedUpdatedAt);
                if (!lock.ok) throw new AppError(CONFLICT_MSG, 'CONFLICT');
                await HistoryService.log('SYSTEM', id, 'UPDATE_CATALOG', {
                    message: `Medio de acceso actualizado: ${payload.name}`,
                    entityName: `Medio de acceso: ${payload.name}`,
                });
            } else {
                ...
                mediaTypeId = data.id;
                await HistoryService.log('SYSTEM', data.id, 'CREATE_CATALOG', {
                    message: `Medio de acceso creado: ${data.name}`,
                    entityName: `Medio de acceso: ${data.name}`,
                });
            }

            // Sincronizar edificios donde aplica el medio (N:M).
            if (mediaTypeId && buildingIds.length > 0) {
                await supabase.from('access_media_type_buildings').delete().eq('media_type_id', mediaTypeId);
                const { error: jError } = await supabase.from('access_media_type_buildings').insert(
    ...
    async deleteCatalogItem(table: string, id: number | string, itemName: string) {
        return withErrorHandling(async () => {
            ...
            if (error) throw error;
            await HistoryService.log('SYSTEM', String(id), 'DELETE_CATALOG', {
                message: `Eliminado de ${table}: ${itemName}`,
                entityName: `${table}: ${itemName}`,
            });
            catalogCache.invalidate(table);
            if (table === 'access_media_types') {
                catalogCache.invalidate('access_media_types_v2');
