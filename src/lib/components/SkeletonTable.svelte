<script lang="ts">
    ...
    let { columns = 4, rows = 5, hasActions = false, className = '' }: Props = $props();

    // Header bar widths per column index
    const headerWidths = ['w-32', 'w-44', 'w-24', 'w-28', 'w-20', 'w-36', 'w-24'] as const;
...
<div class="relative overflow-hidden divide-y divide-slate-100/60 {className}">
    <!-- Shimmer overlay -->
    <div
        class="absolute inset-0 -translate-x-full animate-shimmer bg-[linear-gradient(90deg,transparent,rgba(255,255,255,0.4),transparent)] pointer-events-none"
    ></div>
