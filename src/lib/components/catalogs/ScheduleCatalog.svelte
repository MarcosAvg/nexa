...
{#snippet mobileList(rows: any[])}
    ...
    >
        ...
        {#snippet subtitle(r: any)}
            <span>{r.days?.length ?? 0} días</span>
        {/snippet}
        {#snippet details(r: any)}
            <div class="space-y-3">
                <div>
                    <div class="text-[11px] font-bold uppercase tracking-wider text-slate-400 mb-1">Días</div>
                    <div class="flex flex-wrap gap-1">
                        {#each r.days as day}
                            <span
...
>
    <div class="space-y-4">
        <div>
            <label for="schedule-name" class="block text-sm font-medium text-slate-700 mb-1"
            ...
            <div class="grid grid-cols-2 gap-2">
                {#each ['Lunes', 'Martes', 'Miércoles', 'Jueves', 'Viernes', 'Sábado', 'Domingo'] as day}
                    ...
                    >
                        <input
                            type="checkbox"
                            class="w-4 h-4 text-blue-600 rounded border-slate-300 focus-visible:ring-blue-500"
