...
<div>
    ...
    <div class="flex items-center gap-2 overflow-x-auto pb-4 scrollbar-none">
        {#each buildings as b}
            <button
                type="button"
                class="flex items-center gap-2 px-4 py-2 rounded-xl text-[12px] font-extrabold whitespace-nowrap transition-all duration-300 active:scale-95 {Number(
                    b.id,
                ) === selectedBuildingId
            ...
            >
                <span
                    class="w-1.5 h-1.5 rounded-full {Number(b.id) === selectedBuildingId
                        ? 'bg-white'
                        : 'bg-slate-300'}"
                ></span>
                {b.name}
...
>
    <div class="space-y-4">
        <div>
            <label for="access-name" class="block text-sm font-medium text-slate-700 mb-1"
