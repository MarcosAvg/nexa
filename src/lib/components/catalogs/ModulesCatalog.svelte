    ...
    <div class="flex justify-between items-center mb-8">
        <div>
            ...
            <p class="text-sm font-medium text-slate-500 mt-0.5">
                Activa y configura los flujos de la plataforma por medio de acceso
            </p>
        </div>
            ...
            <Card class="p-6 bg-white border border-slate-200 rounded-2xl shadow-sm">
                <div class="flex items-start justify-between gap-4">
                    <div class="flex items-start gap-3">
                        <div
                            class="w-11 h-11 rounded-2xl bg-slate-100 flex items-center justify-center text-slate-600 shrink-0"
                        >
                ...
                {#if enabled}
                    <div class="mt-5 grid grid-cols-1 sm:grid-cols-2 gap-4 border-t border-slate-100 pt-4">
                        {#each fieldsFor(def, id) as field}
                            ...
                            <div>
                                <label for={fieldId} class="block text-xs font-bold text-slate-600 mb-1"
                                    >{field.label}</label
                                >
