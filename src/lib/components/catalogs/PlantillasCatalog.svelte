<script lang="ts">
    ...
    function removeParagraph(i: number) {
        paragraphs = paragraphs.filter((_, idx) => idx !== i);
        if (paragraphs.length === 0) paragraphs = [''];
    }
    ...
    function previewText(paragraph: string): string {
        return PLACEHOLDERS.reduce(
            (acc, ph) => acc.split(ph.token).join(EJEMPLO[ph.token] ?? ph.token),
            paragraph,
        );
    }
    ...
    async function saveTemplate() {
        ...
        if (cleaned.length === 0) {
            toast.error('Agrega al menos un párrafo de contenido');
            return;
        }
        try {
            if (editingId) {
    ...
    async function toggleActive(t: DocumentTemplate) {
        try {
            ...
            await documentService.setTemplateActive(t.id, next);
            await loadTemplates();
...
<div>
    ...
    <div class="lg:hidden">
        {#if isLoading}
            <div class="p-8 text-center text-sm text-slate-400">Cargando plantillas…</div>
        {:else if templates.length === 0}
            <div class="p-8 text-center">
                <FileText size={36} class="mx-auto text-slate-300 mb-2" />
            ...
            <DataList
                ...
                actions={(t: any) => [
                    ...
                    },
                    {
                        label: 'Duplicar',
                        tone: 'slate' as const,
                        onAction: () => duplicateTemplate(t),
                    },
                    {
                        label: 'Estado',
                        tone: 'emerald' as const,
                        onAction: () => toggleActive(t),
                    },
                    {
                        label: 'Eliminar',
                        tone: 'rose' as const,
                        onAction: () => openDeleteModal(t),
                    },
                ]}
            >
                {#snippet title(t: any)}
                ...
                {#snippet subtitle(t: any)}
                    <span>
                        {mediaName((t as any).media_type_id)} · {t.document_type} · v{t.version}
                    </span>
                {/snippet}
                ...
                {/snippet}
                {#snippet details(t: any)}
                    <div class="space-y-3 text-sm">
                        <div class="flex items-center justify-between">
                            <span class="text-[11px] font-bold uppercase tracking-wider text-slate-400">
                                Medio
                            </span>
                            <span>{mediaName((t as any).media_type_id)}</span>
                        </div>
                        <div class="flex items-center justify-between">
                            <span class="text-[11px] font-bold uppercase tracking-wider text-slate-400">
                                Tipo
                            </span>
                            <span class="capitalize">{t.document_type}</span>
                        </div>
                        <div class="flex items-center justify-between">
                            <span class="text-[11px] font-bold uppercase tracking-wider text-slate-400">
                                Versión
                ...
                {#snippet sheetActions(t: any)}
                    <div class="flex items-center justify-end gap-1">
                        ...
                        <IconButton
                            icon={Power}
                            label={t.active !== false ? 'Desactivar' : 'Activar'}
                            tone="emerald"
                            size="sm"
    ...
    <div class="hidden lg:block bg-white border border-slate-200 rounded-2xl overflow-hidden">
        {#if isLoading}
        ...
        {:else}
            <table class="w-full text-sm">
                <thead>
                    <tr
                        class="text-left text-[10px] font-black uppercase tracking-wider text-slate-400 border-b border-slate-100"
                    >
...
>
    <div class="space-y-5">
        <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
            ...
            <div>
                <label for="tpl-type" class="block text-sm font-medium text-slate-700 mb-1"
                    >Tipo de documento</label
                >
                <select
                    id="tpl-type"
                    bind:value={documentType}
                    class="w-full h-10 px-3 rounded-xl border border-slate-200 bg-white text-sm font-medium text-slate-700 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-blue-500/20 focus-visible:border-blue-500/50"
                >
            ...
            <div>
                <label for="tpl-media" class="block text-sm font-medium text-slate-700 mb-1"
                    >Medio asociado</label
                >
                <select
                    id="tpl-media"
        ...
        <label
            class="flex items-center justify-between gap-3 p-3 rounded-xl border border-slate-200 bg-slate-50/50 cursor-pointer"
        >
        ...
        <div>
            <div class="flex items-center justify-between mb-2">
                ...
                <Button variant="secondary" size="sm" onclick={addParagraph}
                    ><Plus size={14} strokeWidth={3} class="mr-1" /> Párrafo</Button
                >
            </div>
                    ...
                    <div class="p-2 rounded-xl border border-slate-200 bg-white">
                        <div class="flex items-center justify-between mb-1.5">
                            ...
                            <div class="flex gap-1">
                                ...
                                <button
                                    class="p-1 text-slate-400 hover:text-rose-500 disabled:opacity-30"
                        ...
                        <textarea
                            ...
                            class="w-full px-3 py-2 border border-slate-200 rounded-lg text-sm text-slate-700 focus-visible:outline-none focus-visible:border-blue-500 focus-visible:ring-1 focus-visible:ring-blue-500 resize-y"
                        ></textarea>
                        {#if previewing}
                            <p class="mt-1.5 text-[11px] text-slate-500 leading-relaxed whitespace-pre-wrap">
        ...
        <div class="flex items-center gap-3">
            <Button variant="secondary" size="sm" onclick={() => (previewing = !previewing)}>
                ...
                {previewing ? 'Ocultar vista previa' : 'Vista previa'}
            </Button>
            {#if previewing}
                <p class="text-[11px] text-slate-400">
                    Los tokens se muestran reemplazados con datos de ejemplo.
                </p>
            {/if}
        </div>
    </div>
    ...
    {#snippet footer()}
        ...
        <Button variant="primary" onclick={saveTemplate}
            >{editingId ? 'Actualizar plantilla' : 'Crear plantilla'}</Button
        >
    {/snippet}
</Modal>
