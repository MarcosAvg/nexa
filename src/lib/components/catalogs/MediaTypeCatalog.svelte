...
<div>
    ...
    {#snippet renderBuildings(row: any)}
        <div class="flex flex-wrap gap-1.5">
            {#each buildingsByMedia[row.id] || [] as bid}
                <span
                    class="px-2 py-0.5 bg-slate-100 border border-slate-200/60 rounded-lg text-[10px] font-bold text-slate-500"
                >
    ...
    {#snippet renderHasFloors(row: any)}
        {#if row.has_floors}
            <span
                class="inline-flex items-center gap-1.5 text-[11px] font-bold text-amber-700 bg-amber-50 border border-amber-200/60 px-2.5 py-1 rounded-lg"
            >
                <Layers size={12} /> Pisos
            </span>
        {:else}
            <span
                class="inline-flex items-center gap-1.5 text-[11px] font-bold text-slate-400 bg-slate-50 border border-slate-200/60 px-2.5 py-1 rounded-lg"
            >
                Sin pisos
            </span>
        {/if}
    ...
    {#snippet renderActive(row: any)}
        <Badge variant={row.active ? 'emerald' : 'slate'}>
            <span class="inline-flex items-center gap-1"
                ><Power size={11} /> {row.active ? 'Activo' : 'Inactivo'}</span
            >
        </Badge>
    {/snippet}
...
{#snippet mobileList(rows: any[])}
    ...
    >
        ...
        {#snippet subtitle(r: any)}
            <span class="uppercase">{r.key}</span>
        {/snippet}
        {#snippet trailing(r: any)}
            <Badge variant={r.active ? 'emerald' : 'slate'}>
                <span class="inline-flex items-center gap-1"
                    ><Power size={11} /> {r.active ? 'Activo' : 'Inactivo'}</span
                >
            </Badge>
        {/snippet}
        {#snippet details(r: any)}
            <div class="space-y-3">
                <div>
                    ...
                    <div class="flex flex-wrap gap-1.5">
                        {#each buildingsByMedia[r.id] || [] as bid}
                            <span
                                class="px-2 py-0.5 bg-slate-100 border border-slate-200/60 rounded-lg text-[10px] font-bold text-slate-500"
                            >
                                {buildings.find((b) => Number(b.id) === bid)?.name ?? bid}
...
>
    <div class="space-y-4">
        <div>
            <label for="media-name" class="block text-sm font-medium text-slate-700 mb-1"
                >Nombre del Medio</label
            >
            <Input
                id="media-name"
                placeholder={editingId ? 'Nombre' : 'Ej. P2000, KONE, AccessPRO…'}
        ...
        <div>
            ...
            <div class="flex flex-wrap gap-2">
                {#each buildings as b}
                    ...
                    <button
                        type="button"
                        class="px-3 py-1.5 rounded-xl text-[11px] font-bold border-2 transition-all active:scale-95 {mediaBuildings.includes(
                            bid,
                        )
        ...
        <div>
            ...
            <p class="text-[11px] text-slate-400 mt-1.5">
                Personaliza el color con el que se muestra este medio en la app.
            </p>
        </div>
        <div class="space-y-2 pt-1">
            ...
            <ToggleRow
                ...
                checked={mediaRequiresProgramming}
                onChange={(v) => (mediaRequiresProgramming = v)}
            />
            <ToggleRow
                label="Requiere responsiva"
                checked={mediaRequiresResponsiva}
                onChange={(v) => (mediaRequiresResponsiva = v)}
            />
            <ToggleRow
                label="Requiere identificador (folio)"
                checked={mediaRequiresIdentifier}
                onChange={(v) => (mediaRequiresIdentifier = v)}
            />
            {#if editingId}
