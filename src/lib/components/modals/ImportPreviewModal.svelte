<script lang="ts">
    ...
    import { analyzeAltaConflicts, analyzeModificacionConflicts, wantsCard } from '../../utils/matchAnalysis';
    ...
    function validateRowAgainstCatalog(sheetKey: string, row: ParsedRow): string[] {
        const f = row.fields;
        ...
        if (sheetKey === 'altas') {
            ...
            if (f.edificio && catalogHasName(cat.buildings, f.edificio)) {
                ...
                if (f.piso_base) {
                    ...
                    if (badBase.length)
                        problems.push(`Piso base "${f.piso_base}" no existe en ${f.edificio}`);
                }
                for (const m of activeMediaTypes(cat.mediaTypes)) {
                    ...
                    if (unresolved.length) {
                        problems.push(`Pisos ${m.name}: ${unresolved.join(', ')}`);
                    }
                }
            }
            ...
            if (targetBuilding) {
                ...
                if (f.nuevo_piso) {
                    ...
                    if (badBase.length)
                        problems.push(`Nuevo piso base "${f.nuevo_piso}" no existe en ${targetBuilding}`);
                }
                for (const m of activeMediaTypes(cat.mediaTypes)) {
                    if (!m.has_floors) continue;
    ...
    async function startReview() {
        ...
        for (const sheet of parseResult.sheets) {
            for (const row of sheet.rows) {
                ...
                if (sheet.key === 'altas') {
                    newAltaAnalyses.set(rk, analyzeAltaConflicts(rk, person, row.fields, mediaTypes));
                } else if (sheet.key === 'modificaciones') {
                    newModAnalyses.set(rk, analyzeModificacionConflicts(rk, person, row.fields, mediaTypes));
                }
            }
        }
        ...
        // Auto-seleccionar la primera hoja con filas seleccionadas
        // (p.ej. un archivo con solo ALTAS ACCESSPRO no debe abrir en ALTAS)
        const firstSheetWithRows = parseResult.sheets.find((s) =>
            s.rows.some((r) => r.isValid && selectedRows.has(`${s.key}-${r.rowNumber}`)),
        );
        const currentTabHasRows = parseResult.sheets
            .find((s) => s.key === reviewSheetTab)
            ?.rows.some((r) => r.isValid && selectedRows.has(`${reviewSheetTab}-${r.rowNumber}`));
        if (!currentTabHasRows && firstSheetWithRows) {
            reviewSheetTab = firstSheetWithRows.key;
        }
    ...
    function getReposicionCards(fields: Record<string, string>): { type: string; folio: string }[] {
        const cards: { type: string; folio: string }[] = [];
        const yes = ['sí', 'si'];
        for (const media of activeMediaTypes(mediaTypes)) {
            if (yes.includes((fields[`reponer_${media.key}`] ?? '').toLowerCase())) {
                cards.push({
                    type: media.name,
    ...
    function getModificationChanges(fields: Record<string, string>): { label: string; value: string }[] {
        ...
        for (const media of activeMediaTypes(mediaTypes).filter((m) => m.has_floors)) {
            if (fields[`accion_${media.key}`]) {
                changes.push({
                    label: `Acción ${media.name}`,
                    value: `${fields[`accion_${media.key}`]}: ${fields[`pisos_${media.key}`] || 'N/A'}`,
                });
            }
        }
        if (fields.accion_acc) {
            const accesses =
                [fields.acceso1, fields.acceso2, fields.acceso3].filter(Boolean).join(', ') || 'N/A';
            changes.push({
    ...
    function getActiveCards(person: Person): any[] {
        return ((person as any).cards ?? []).filter((c: any) => c.status === 'active');
    }
     ...
     */
    function resolvePersonId(sheetKey: SheetKey, rowNumber: number): string | null {
     ...
     */
    function buildTicketDef(sheetKey: SheetKey, row: ParsedRow, fieldsOverride?: Record<string, string>) {
        ...
        if (fields.urgencia) {
            ...
            if (urg.includes('alta') || urg.includes('urgente')) priority = 'alta';
     ...
     */
    function applyAltaResolutions(row: ParsedRow, analysis: AltaConflictAnalysis, sheetKey: SheetKey): any[] {
        ...
        for (const conflict of analysis.conflicts) {
            ...
            if (conflict.resolution === 'skip_card' || conflict.resolution === 'convert_to_reposicion') {
                ...
                if (conflict.resolution === 'convert_to_reposicion') {
                    extraTickets.push({
                        ...
                        payload: {
                            ...
                            [`folio_${conflict.mediaKey}`]: conflict.existingFolio || '',
                            origen: 'Conversión desde Alta (matching inteligente)',
                        },
                        person_id: resolvePersonId(sheetKey, row.rowNumber),
                    });
                }
        ...
        const stillWants =
            activeMediaTypes(mediaTypes).filter((m) => wantsCard(modifiedFields, m)).length > 0;
    ...
    function toggleSheetSelection(sheet: ParsedSheet) {
        const rowKeys = sheet.rows.filter((r) => r.isValid).map((r) => `${sheet.key}-${r.rowNumber}`);
    ...
    async function handleImport() {
        ...
        if (blockedRows.length > 0) {
            ...
            const sheetLabel =
                sheetKey === 'altas' ? 'Altas' : sheetKey === 'modificaciones' ? 'Modificaciones' : sheetKey;
        ...
        for (const sheet of parseResult.sheets) {
            for (const row of sheet.rows) {
                ...
                if (sheet.key === 'altas' && altaAnalyses.has(rowKey)) {
                    ...
                    const result = applyAltaResolutions(row, analysis, sheet.key);
                    tickets.push(...result);
                } else if (sheet.key === 'modificaciones' && modAnalyses.has(rowKey)) {
        ...
        try {
            ...
            if (result.created > 0) {
                toast.success(`${result.created} ticket(s) creados correctamente.`);
                onComplete?.();
            }
            if (result.errors.length > 0) {
                toast.error(`${result.errors.length} ticket(s) no pudieron crearse.`);
            }
        } catch (err) {
    ...
    async function handleExportReport() {
        ...
        try {
            ...
            const { exportConflictReportToExcel } = await import('../../utils/xlsxConflictReport');
    ...
    const SHEET_COLORS: Record<string, 'emerald' | 'amber' | 'rose' | 'blue' | 'violet' | 'slate'> = {
        altas: 'emerald',
    ...
    function cardTypeClasses(type: string): string {
        return (
            VARIANT_BADGE_CLASSES[mediaTypeVariant(type)] ?? 'bg-slate-100 text-slate-600 border-slate-200'
        );
    }
...
>
    <div class="space-y-5">
                ...
                <div class="grid grid-cols-3 gap-3 text-center">
                    <div class="rounded-lg p-3 bg-slate-50 border border-slate-200">
                        <p class="text-2xl font-bold text-slate-800">
                            {parseResult.sheets.reduce((s, sh) => s + sh.rows.length, 0)}
                        </p>
                        ...
                        <div class="rounded-lg border border-slate-200 overflow-hidden">
                            ...
                            >
                                <input
                                    ...
                                    checked={sheet.rows
                                        .filter((r) => r.isValid)
                                        .every((r) => selectedRows.has(`${sheet.key}-${r.rowNumber}`))}
                                    indeterminate={sheet.rows
                                        .filter((r) => r.isValid)
                                        .some((r) => selectedRows.has(`${sheet.key}-${r.rowNumber}`)) &&
                                        !sheet.rows
                                            .filter((r) => r.isValid)
                                            .every((r) => selectedRows.has(`${sheet.key}-${r.rowNumber}`))}
                                ...
                                >
                                    <div class="flex items-center gap-2">
                                        <Badge variant={SHEET_COLORS[sheet.key] ?? 'slate'}
                                            >{sheet.label}</Badge
                                        >
                                        <span class="text-xs text-slate-500">{sheet.rows.length} fila(s)</span
                                        >
                                    </div>
                                    <div class="flex items-center gap-2">
                                        ...
                                        >
                                            {#if row.isValid}
                                                <input
                                                    ...
                                                    onchange={() => toggleRow(sheet.key, row.rowNumber)}
                                                />
                                            {:else}
                                                <AlertCircle
                                                    size={14}
            ...
            {#if isReviewing}
                <div class="flex flex-col items-center gap-4 py-12">
                    ...
                    <div class="text-center">
                        ...
                        <p class="text-xs text-slate-400 mt-1">
                            Validando {totalSelected} persona(s) contra la base de datos.
                        </p>
                    </div>
                </div>
            {:else if parseResult}
                <!-- Barra de resumen de revisión -->
                ...
                {@const totalConflicts = [...altaAnalyses.values()].filter((a) => a.hasConflicts).length}
                ...
                <div class="flex gap-1.5 flex-wrap pb-2 px-1">
                    {#each sheetTabs as tab}
                        {@const tabCount =
                            parseResult.sheets
                                .find((s) => s.key === tab.key)
                                ?.rows.filter(
                                    (r) => r.isValid && selectedRows.has(`${tab.key}-${r.rowNumber}`),
                                ).length ?? 0}
                        {#if tabCount > 0}
                            <button
                                class="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-bold transition-colors {reviewSheetTab ===
                                tab.key
                ...
                {@const sheetOkCount =
                    currentSheet?.rows.filter(
                        (r) =>
                            r.isValid &&
                            selectedRows.has(`${reviewSheetTab}-${r.rowNumber}`) &&
                            rowMatchesTab(reviewSheetTab, r, 'ok'),
                    ).length ?? 0}
                {#if sheetConflictCount > 0 || sheetOkCount > 0}
                    <div class="flex gap-2 px-1 pb-2">
                        <button
                            class="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-bold transition-colors {reviewTab ===
                            'conflict'
                                ...
                                {#each sheetRows as row}
                                    ...
                                    <div class="rounded-xl border border-slate-200 overflow-hidden bg-white">
                                        ...
                                        <button
                                            ...
                                            onclick={() => toggleReviewRow(rowKey)}
                                        >
                                            <div class="flex items-center gap-3 min-w-0">
                                                ...
                                                <div class="min-w-0">
                                                    ...
                                                    <p class="text-[10px] text-slate-400 truncate">
                                                        ...
                                                        {:else if reviewSheetTab === 'altas'}
                                                            · <span class="text-emerald-600 font-medium"
                                                                >Persona nueva — Sin conflictos</span
                                                            >
                                                        {:else}
                                        ...
                                        {#if isExpanded}
                                            <div class="px-4 pb-4 space-y-3 border-t border-slate-100 pt-3">
                                                    ...
                                                    <div class="space-y-3">
                                                                        ...
                                                                        <div class="space-y-1.5">
                                                                            ...
                                                                            >
                                                                                <input
                                                                                    ...
                                                                                    class="w-3.5 h-3.5 text-amber-600 focus-visible:ring-amber-500"
                                                                                />
                                                        ...
                                                        {:else}
                                                            ...
                                                            >
                                                                ...
                                                                <span
                                                                    >Sin conflictos — la persona no tiene
                                                                    tarjetas activas de los tipos solicitados.</span
                                                                >
                                                            </div>
                                                        {/if}
                                                    </div>
                                                    ...
                                                    <div class="space-y-3">
                                                        ...
                                                        >
                                                            ...
                                                            >
                                                                ...
                                                                <div class="bg-amber-50 px-3 py-2"></div>
                                                        ...
                                                        {#if analysis.floorChanges}
                                                            {#each Object.entries(analysis.floorChanges) as [key, fc]}
                                                                {@const isAccesses = key === 'accesses'}
                                                                {@const mediaLabel =
                                                                    activeMediaTypes(mediaTypes).find(
                                                                        (m) => m.key === key,
                                                                    )?.name ?? key}
                                                        ...
                                                        >
                                                            ...
                                                            >
                                                                <input
                                                                    ...
                                                                    onchange={() => {
                                                                        analysis.resolution = 'apply';
                                                                    }}
                                                                    class="w-3 h-3 text-emerald-600 focus-visible:ring-emerald-500"
                                                                />
                                                                <span
                                                                    class="text-xs font-semibold text-emerald-700"
                                                                    >Crear Ticket de Modificación</span
                                                                >
                                                            </label>
                                                            <label
                                                            ...
                                                            >
                                                                <input
                                                                    type="radio"
                                                                    name="mod-resolve-{rowKey}"
                                                                    checked={analysis.resolution === 'reject'}
                                                                    onchange={() => {
                                                                        analysis.resolution = 'reject';
                                                                    }}
                                                                    class="w-3 h-3 text-rose-600 focus-visible:ring-rose-500"
                                                                />
                                                                <span
                                                                    class="text-xs font-semibold text-rose-700"
                                                                    >Rechazar y omitir</span
                                                                >
                                                            </label>
                                                        </div>
                                                    </div>
                                                ...
                                                {:else if reviewSheetTab === 'altas'}
                                                    ...
                                                    {@const rowProblems = validationErrors.get(rowKey) ?? []}
                                                        ...
                                                        >
                                                            <div class="flex items-center gap-2">
                                                                ...
                                                                <div>
                                                                    ...
                                                                    >
                                                                        Datos no reconocidos en el catálogo
                                                                    </p>
                                                                    <p class="text-[10px] text-rose-600">
                                                                        El alta no puede continuar hasta
                                                            ...
                                                            <div class="flex items-center gap-2">
                                                                ...
                                                                <div>
                                                                    ...
                                                                    <p class="text-[10px] text-emerald-600">
                                                                        No se encontró en el sistema — el alta
                                                                        puede continuar sin conflictos.
                                                                    </p>
                                                                </div>
                                                            </div>
                                                ...
                                                {:else if reviewSheetTab === 'modificaciones'}
                                                    ...
                                                    {:else}
                                                        <p class="text-[10px] text-slate-400 italic">
                                                            Sin cambios específicos detectados en los campos.
                                                        </p>
                                                    {/if}
                                                ...
                                                {/if}
                                                <!-- ── MATCH RESULTS (only for types WITHOUT smart analysis) ── -->
                                                {#if !(reviewSheetTab === 'altas') && !(reviewSheetTab === 'modificaciones' && modAnalyses.has(rowKey))}
                                                                                    ...
                                                                                    {#if rc.folio && rc.folio !== existingCard.folio}
                                                                                        ...
                                                                                        >
                                                                                            <AlertTriangle
                                                                                                size={10}
                                                                                            />
                                                                                            Folio en plantilla ({rc.folio})
                                                                                            ≠ folio asignado ({existingCard.folio})
                                                                                        </p>
                                                                                    {:else}
                                                                                        <p
                                                    ...
                                                    {:else}
                                                        ...
                                                        >
                                                            ...
                                                            <span
                                                                >No se encontró a "<strong
                                                                    >{row.fields.apellidos},
                                                                    {row.fields.nombres}</strong
                                                                >" en la base de datos.
                                                                {#if reviewSheetTab === 'altas'}
                                                                    Se creará como persona nueva al procesar
                                                                    el ticket.
                                                                {:else}
                                                                    Será necesario vincular manualmente al
                                                                    procesar el ticket.
                                                                {/if}
                                                            </span>
                                                        </div>
        ...
        {#if step === 'importing'}
            <div class="flex flex-col items-center gap-4 py-12">
                ...
                <div class="text-center">
                    <p class="text-sm font-semibold text-slate-700">Importando tickets…</p>
                    <p class="text-xs text-slate-400 mt-1">
        ...
        {#if step === 'done' && importResult}
            <div class="flex flex-col items-center gap-3 py-8">
                ...
                <p class="text-base font-bold text-slate-800">Importación completada</p>
                <p class="text-sm text-slate-500">
                    Se crearon <span class="font-bold text-emerald-600">{importResult.created}</span> ticket(s)
                    correctamente.
                </p>
    ...
    {#snippet footer()}
        <div class="flex items-center justify-between w-full">
            <Button variant="ghost" class="text-slate-400 hover:text-slate-600" onclick={closeModal}>
            ...
            {:else if step === 'parsed'}
                <div class="flex items-center gap-3">
                    ...
                    <Button variant="primary" disabled={totalSelected === 0} onclick={startReview}>
