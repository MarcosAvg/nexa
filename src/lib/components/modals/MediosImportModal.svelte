<script lang="ts">
    ...
    import type { ParsedRow } from '../../utils/xlsxFields';
    ...
    let validation = $state<
        {
            ...
            folio: string;
            tipo: string;
            status: 'ok' | 'duplicate_file' | 'exists_db' | 'invalid_tipo' | 'missing_folio';
            message: string;
        }[]
    >([]);
        ...
        try {
            ...
            for (let i = 0; i < payload.length; i += 100) {
                ...
                const { data, error } = await supabase
                    .from('access_media')
                    .insert(batch)
                    .select('id, identifier, media_type_id');
                if (error) {
...
<Modal bind:isOpen title="Importar Medios — Inventario" size="xl" onclose={closeModal}>
    {#if step === 'idle'}
        <div class="space-y-6">
            <div class="rounded-xl border-2 border-dashed border-slate-200 p-8 text-center bg-slate-50/50">
                <div
                    class="w-14 h-14 rounded-xl bg-sky-50 text-sky-600 flex items-center justify-center mx-auto mb-3"
                >
                    <FileSpreadsheet size={28} />
                </div>
                <p class="text-sm font-bold text-slate-800">Plantilla separada de Medios</p>
                <p class="text-xs text-slate-500 mt-1">
                    Columnas: <b>Tipo</b> (desplegable con tipos vigentes) y <b>Folio</b>. Cada fila es un
                    medio disponible.
                </p>
                <input
                    bind:this={fileInput}
        ...
        <div class="space-y-4">
            ...
            <div class="grid grid-cols-3 gap-3">
                ...
                <div class="rounded-xl border border-rose-200 bg-rose-50 p-3 text-center">
                    <div class="text-[10px] font-bold uppercase tracking-widest text-rose-600">
                        Conflictos
                    </div>
            ...
            <div
                class="flex flex-col sm:flex-row gap-2 sm:items-center justify-between bg-slate-50 border border-slate-200 rounded-xl p-2.5"
            >
                <div class="flex items-center gap-1.5">
                    <div class="relative">
                        ...
                        <input
                            ...
                            aria-label="Filtrar por tipo o folio"
                            bind:value={searchQuery}
                            class="pl-8 pr-3 py-1.5 rounded-lg border border-slate-200 bg-white text-xs w-full sm:w-56 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-violet-200 focus-visible:border-violet-300"
                        />
                    </div>
                    <select
                        bind:value={statusFilter}
                        aria-label="Filtrar por estado"
                        class="px-2.5 py-1.5 rounded-lg border border-slate-200 bg-white text-xs font-medium"
                    >
                ...
                <div class="flex items-center gap-1.5">
                    {#if totalConflict > 0}
                        <Button
                            variant="ghost"
                            size="sm"
                            class="h-7 text-xs text-rose-600 hover:bg-rose-50 hover:text-rose-700"
            ...
            <div class="rounded-xl border border-slate-200 overflow-x-auto bg-white">
                <div class="min-w-[560px]">
                    <div
                        class="grid grid-cols-[44px_1fr_1fr_160px_44px] gap-px bg-slate-200 text-[10px] font-bold uppercase tracking-wider"
                    >
                        <div class="bg-slate-50 px-2 py-2.5 text-center">#</div>
                        <div class="bg-slate-50 px-3 py-2.5">
                            Tipo <span class="normal-case font-normal text-slate-400">(desplegable)</span>
                        </div>
                    ...
                    <div class="max-h-[320px] overflow-auto divide-y divide-slate-100">
                        {#each filteredValidation as v, idx}
                            <div
                                class="grid grid-cols-[44px_1fr_1fr_160px_44px] gap-px bg-slate-100 text-xs items-center {v.status !==
                                'ok'
                                    ? 'bg-rose-50/30'
                                    : 'bg-white'}"
                            >
                                <div class="bg-white px-2 py-2.5 text-center">
                                    <span class="font-bold text-slate-700" title="Fila Excel {v.rowNumber}"
                                        >{idx + 1}</span
                                    >
                                ...
                                <div class="bg-white px-3 py-2.5">
                                    <span
                                        class="font-semibold {v.status === 'invalid_tipo'
                                            ? 'text-rose-600'
                                            : 'text-slate-700'}">{v.tipo || '—'}</span
                                    >
                                </div>
                                <div
                                    class="bg-white px-3 py-2.5 font-mono text-[11px] font-medium text-slate-700"
                                >
                                    {v.folio || '—'}
                                </div>
                    ...
                    <div
                        class="bg-slate-50 px-3 py-2 flex items-center justify-between text-[11px] text-slate-500 border-t border-slate-200"
                    >
                        <span>Mostrando {filteredValidation.length} de {validation.length} filas</span>
                        <span class="hidden sm:inline"
                            >Tip: usa el desplegable “Tipo” de la plantilla para evitar errores de escritura.</span
                        >
                    </div>
                </div>
            </div>
            ...
            {#if totalConflict > 0}
                <div class="rounded-lg bg-amber-50 border border-amber-200 p-3 flex gap-2.5">
                    ...
                    <div class="text-xs text-amber-800">
                        ...
                        <p>
                            Las filas con conflicto no se importarán. Puedes quitarlas con “Quitar conflictos”
                            o eliminar individualmente con <Trash2 size={10} class="inline" />. Corrige el
                            Excel y vuelve a subirlo si necesitas conservarlas.
                        </p>
                    </div>
                </div>
            {/if}
    ...
    {:else if step === 'done' && importResult}
        <div class="text-center py-6 space-y-3">
            <div
                class="w-14 h-14 rounded-full bg-emerald-100 text-emerald-600 flex items-center justify-center mx-auto"
            >
            ...
            {#if importResult.errors.length > 0}
                <div
                    class="text-left rounded-lg bg-rose-50 border border-rose-200 p-3 max-h-40 overflow-auto"
                >
    ...
    {#snippet footer()}
        {#if step === 'review'}
            ...
            <Button
                ...
                disabled={isImporting || totalOk === 0}
                loading={isImporting}
            >
