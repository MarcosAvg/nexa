...
<aside
    class="fixed inset-y-0 left-0 z-50 flex h-full {uiState.isSidebarCondensed
        ? 'w-20'
        : 'w-72'} flex-col bg-slate-950 text-slate-300 transition-all duration-300 ease-in-out lg:translate-x-0 {uiState.isSidebarOpen
        ? 'translate-x-0'
...
>
    ...
    <!-- Header/Logo Area -->
    <div
        class="flex h-24 items-center flex-shrink-0 {uiState.isSidebarCondensed
            ? 'justify-center px-0'
            : 'px-8 justify-between'} mb-4 relative transition-all duration-300"
    ...
    >
        <div style="-webkit-app-region: no-drag;" class="flex items-center gap-2">
            <Logo
                showText={!uiState.isSidebarCondensed}
                class="{uiState.isSidebarCondensed ? 'scale-125' : 'scale-110 origin-left'} transition-all"
            />
        </div>

        <div style="-webkit-app-region: no-drag;">
            <button
                class="hidden lg:flex items-center justify-center {uiState.isSidebarCondensed
                    ? 'absolute -right-3 top-10 h-6 w-6 bg-slate-800 border border-slate-700 rounded-full z-50 text-slate-400 hover:text-white hover:bg-slate-700'
                    : 'p-2 text-slate-500 hover:text-white rounded-lg hover:bg-white/5'} transition-all duration-300"
                onclick={() => uiState.toggleSidebarCondensed()}
                title={uiState.isSidebarCondensed ? 'Expandir menú' : 'Colapsar menú'}
                aria-label={uiState.isSidebarCondensed ? 'Expandir menú' : 'Colapsar menú'}
            >
    ...
    <!-- Navigation -->
    <nav class="flex-1 overflow-y-auto px-5 py-2 space-y-2 custom-scrollbar relative">
        {#if !uiState.isSidebarCondensed}
            <div class="px-4 mb-5 flex items-center gap-3 transition-opacity duration-300">
                <p
                    class="text-[10px] font-extrabold uppercase tracking-[0.25em] text-slate-500/70 whitespace-nowrap"
                >
                    Menú Principal
                </p>
        ...
        {#each items as item}
            <a
                ...
                class="group relative flex items-center gap-4 rounded-2xl {uiState.isSidebarCondensed
                    ? 'px-0 justify-center w-12 mx-auto'
                    : 'px-5'} py-3.5 text-[13.5px] font-extrabold transition-all duration-300 text-slate-400 hover:bg-white/[0.02] hover:text-white tracking-tight"
            ...
            >
                ...
                <div
                    class="absolute left-0 top-1/2 -translate-y-1/2 w-1.5 h-7 bg-blue-500 {uiState.isSidebarCondensed
                        ? 'rounded-full -ml-3'
                        : 'rounded-r-full'} transition-all duration-500 scale-y-0 group-[.active-nav-item]:scale-y-100 opacity-0 group-[.active-nav-item]:opacity-100 shadow-[0_0_20px_rgba(59,130,246,0.8)]"
                ></div>
                ...
                {#if !uiState.isSidebarCondensed}
                    ...
                    <div class="ml-auto opacity-0 group-[.active-nav-item]:opacity-100 transition-opacity">
                        <div
    ...
    {#if user}
        <div
            class="mt-auto {uiState.isSidebarCondensed
                ? 'p-3'
                : 'p-5'} relative border-t border-white/5 transition-all duration-300"
        >
            {#if userState.profile?.role === 'admin'}
                <button
                    onclick={() => uiState.toggleDirectEditMode()}
                    class="w-full flex items-center {uiState.isSidebarCondensed
                        ? 'justify-center px-0'
                        : 'gap-3 px-4'} py-2.5 mb-3 rounded-xl transition-all duration-300 border {uiState.isDirectEditMode
                        ? 'bg-amber-500/10 border-amber-500/30 text-amber-500 shadow-[inset_0_0_12px_rgba(245,158,11,0.2)]'
                        : 'bg-transparent border-transparent text-slate-500 hover:bg-slate-800/50 hover:text-slate-300'}"
                    title={uiState.isSidebarCondensed
                        ? uiState.isDirectEditMode
                            ? 'Modo Edición Directa: ACTIVO'
                            : 'Modo Edición Directa: INACTIVO'
                        : undefined}
                >
                    <Wrench
                        size={16}
                        strokeWidth={2.5}
                        class={uiState.isDirectEditMode ? 'animate-pulse' : ''}
                    />
                    {#if !uiState.isSidebarCondensed}
                        ...
                        <div class="ml-auto flex items-center justify-center">
                            <div
                                class="w-8 h-4 rounded-full transition-colors duration-300 relative {uiState.isDirectEditMode
                                    ? 'bg-amber-500'
                                    : 'bg-slate-700'}"
                            >
                                <div
                                    class="absolute top-0.5 w-3 h-3 rounded-full bg-white transition-transform duration-300 {uiState.isDirectEditMode
                                        ? 'translate-x-[18px]'
                                        : 'translate-x-[2px]'}"
                                ></div>
                            </div>
            ...
            {/if}
            <div
                class="flex items-center {uiState.isSidebarCondensed
                    ? 'justify-center p-2'
                    : 'gap-3.5 p-4'} rounded-2xl bg-gradient-to-b from-slate-900/50 to-slate-950/50 border border-white/5 backdrop-blur-md shadow-2xl group transition-all duration-300 hover:border-blue-500/30 hover:shadow-blue-500/5"
            >
                <div
                ...
                >
                    ...
                    {:else}
                        <span class="text-sm tracking-tighter">{user.name.charAt(0).toUpperCase()}</span>
                    {/if}
                </div>

                {#if !uiState.isSidebarCondensed}
                    <div class="flex flex-col min-w-0 flex-1 transition-opacity duration-300">
                        <span class="truncate text-[13.5px] font-extrabold text-white tracking-tight"
                            >{user.name}</span
                        >
                        <span class="truncate text-[11px] font-bold text-slate-500 tracking-tight"
                            >{user.email}</span
                        >
                    </div>
            ...
            {#if !uiState.isSidebarCondensed}
                <div class="mt-5 space-y-2">
                        ...
                        {:else}
                            ...
                            >
                                ...
                                {#if versionState.formattedBuildTime}
                                    <span class="text-[9px] font-medium text-emerald-400/40 ml-auto"
                                        >{versionState.formattedBuildTime}</span
                                    >
                                {/if}
                            </div>
                        {/if}
                    {:else}
                        ...
                        >
                            <div
                                class="w-3 h-3 rounded-full border-2 border-slate-500 border-t-transparent animate-spin"
                            ></div>
