...
const modif = medias
    .filter((m) => m.has_floors)
    .flatMap((m) => modifMediaCols(m))
    .map((c) => c.key);
...
if (!altas.includes('accesspro_req') || !altas.includes('accesspro_folio'))
    fail('ALTAS accesspro: faltan req/folio');
...
if (modif.includes('accion_chip') || modif.includes('pisos_chip'))
    fail('MODIF: medio sin pisos no debe aparecer');
